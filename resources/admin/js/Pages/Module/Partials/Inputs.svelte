    ...
    <div class="md:col-span-2">
        <Card size="xl">
            <div class="w-full grid md:grid-cols-2 gap-2 bg-white">
                ...
                <div class="w-full">
                    <Label for="view_path">
                        Ruta de la vista <sup class="text-red-600">*</sup>
                    </Label>
                    <Input
                        id="view_path"
                        type="text"
                        class="mt-1 block w-full"
                        bind:value={$form.view_path}
                        required
                        autocomplete="view_path"
                    />
                    {#if $form.errors.view_path}
                        <Helper color="red" class="mt-2">
                            {$form.errors.view_path}
                        </Helper>
                    {/if}
                </div>

                <div class="w-full">
                    <ModuleType bind:form></ModuleType>
                    {#if $form.errors.module_type_id}
                        <Helper color="red" class="mt-2">
                            {$form.errors.module_type_id}
                        </Helper>
                    {/if}
                </div>
            </div>
...
<div class="flex items-center mt-4">
    ...
    <Button href="/admin/modules" color="alternative" class="ml-4">
        Cancelar
    </Button>
</div>
